...
| `materialize-epic` | One-command materialization of completed architect and planner outputs into one existing epic, with conservative child defaults and optional active-scope membership; no architecture, planning, worker, or implementation work. Available when Beads and the OpenCode Beads integration are enabled |
...
The hosted [Mend Renovate GitHub App](https://github.com/apps/renovate) manages GitHub-sourced Herdr plugin refs and both Beads runtime fork pins in `.chezmoidata.toml`. The local Elio and Hub Viewer plugins are vendored and are not Renovate dependencies. If the app is not enabled, install it for this GitHub account and grant it access to this repository; no self-hosted workflow is required.
