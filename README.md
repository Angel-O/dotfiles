...
Machines with the previous repository-managed `~/.local/share/beads/work` store can run this explicit one-time migration from the dotfiles source checkout. The script is repository-only and is not run by chezmoi; it renames `work-*` IDs to `bead-*`, preserves repository registrations, and creates a timestamped backup before moving data:
